...
writeSequences <- function(sequence_list, out_name = "sra_enterics-serratia_", mainDir = "~/phd/RNASeq/srna_all/"){
...
dat <- read.table("~/phd/RNASeq/combined_gff_files/enterics-serratia_merged.gff", header = T, sep = "\t",
                  comment.char = "", quote = "", as.is = T )

#dat <- dat %>% filter(new_feature)
...
if(testing == T) {
  ...
  oldData[oldData == "1-1"] <- "1"
  oldData[oldData == "1-0"] <- "0-1"
  
  dat %>%  group_by(klebsiella) %>% summarise(count = n())
...
write.table(sequence_list_all, "~/phd/RNASeq/srna_all/sequence_list.txt", sep = "\t", quote = F, row.names = F)
#ids_seq <- ids_seq%>%mutate(ids_short = paste("sra_enterics-serratia_", row_number(), sep = ""))
i <- 1
j <- 1

sequence_list_all <- read.table("~/phd/RNASeq/srna_all/sequence_list.txt", header = T, sep = "\t", as.is = T)

sequence_list_all_srna <- sequence_list_all%>%filter(nchar(sequence) >= 30, nchar(sequence) <= 500)
write.table(sequence_list_all_srna, "~/phd/RNASeq/srna_all/sequence_list_30-500.txt", sep = "\t", quote = F, row.names = F)
...
writeSequences(sequence_list = sequence_list_all_srna, out_name = "sra_enterics-serratia_TEST-", mainDir = "~/phd/RNASeq/srna_all/new/")
