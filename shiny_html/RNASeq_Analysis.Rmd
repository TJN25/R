...
ids_seq <- ids_seq%>%mutate(ids_short = paste("sra_enterics-serratia_predicted_version_8b_", row_number(), sep = ""))
...
for(i in 1:loop_length){
  for(j in 2:(ncol(ids_seq) -  2)){
    if(ids_seq[i, j] != ""){
      ...
      mat[2,1] <- ids_seq[i,j]
      write.table(mat, file = paste("~/phd/RNASeq/srna_all/version_8/predicted_b/", ids_seq[i, ncol(ids_seq)], ".fasta", sep = ""),
...
save(idsSeq_predicted, file = "idsSeq_predicted_8b.Rda")
