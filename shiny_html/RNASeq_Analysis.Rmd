...
ids_seq <- ids_seq%>%mutate(ids_short = paste("sra_enterics-serratia_known_version_8c_", row_number(), sep = ""))
...
save(idsSeq_predicted, file = "idsSeq_predicted_8c.Rda")
...
alifoldNotConseved <- read.table("~/phd/RNASeq/srna_all/version_8/alifold_not_conserved_all", header = F, comment.char = "#", quote = "", sep = "", fill = T, as.is = T, 
                      col.names = c( "From",      "To",    "Strand",    "Native.MFE",    "Mean.MFE",     "STDV",      "Z"))
alifoldConseved <- read.table("~/phd/RNASeq/srna_all/version_8/alifold_known_b_all", header = F, comment.char = "#", quote = "", sep = "", fill = T, as.is = T, 
                      col.names = c( "From",      "To",    "Strand",    "Native.MFE",    "Mean.MFE",     "STDV",      "Z"))
...
```{r nhmmer}
dat <- read.table("~/phd/RNASeq/srna_all/version_8/known_c/multiple_seqs/nhmmer/alignments/output/all.out", comment.char = "#", fill = T, sep = "", header = F, quote = "", as.is = T)
...
dat <- dat %>% mutate(target.genome = paste(t1, t2, sep = "_"),
              ...
              mutate(target.seq.id = paste(target.sra, t9, sep = "_")) %>% 
        select(-t1, -t2, -t3, -t4, -t5, -t6, -t7, -t8, -t9)
dat <- dat %>% separate(col = query.name, into = c("query.sra"), sep = "\\.", extra = "drop", remove = F)
...
mat <- dcast( datSmall , target.sra~query.sra)
...
datSummary <- datSmall %>% group_by(target.sra) %>% summarise(count = length(unique(query.sra)))
