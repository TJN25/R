...
resList$species <- c("Acinetobacter", "Altermonas", "Enterobacter", "escherichia-shigella", "Klebsiella", "pseudomonas", "serratia", "xanthomonas")
...
resList$family <- c("escherichia-shigella-salmonella-enterobacter-klebsiella-serratia", "acinetobacter-pseudomonas")
resList$order <- c("acinetobacter-pseudomonas-alteromonas", "escherichia-shigella-salmonella-enterobacter-klebsiella-serratia-xanthomonas-lysobacter", 
                   "all_2")
conservationDataFrame <- function(resList, random = F){
if(random){
  filePath <- "~/phd/RNASeq/combined_gff_files_random/version_6/"
}else{
  filePath <- "~/phd/RNASeq/combined_gff_files/version_6/"
}
...
for(i in resList$species){
  ...
  tmp <- data.frame(id = dat$id,
                     ...
                     strain.prop = NA,
                    strain.score = NA,
                     species.fitch = dat$set_val,
                     species.prop = dat$prop,
                    species.score = dat$prop_overlap,
                     genus.fitch = NA,
                     genus.prop = NA,
                    genus.score = NA,
                     family.fitch = NA,
                    ...
                    order.score = NA,
                     file.name = i,
                    category = "species")
        tmp <- tmp %>% mutate(keep = ifelse(abs(species.score) > 0.25, T, ifelse(is.na(species.score), T,F))) %>% mutate_all(as.character) %>% mutate(score = species.score)
  ...
  resDat <- resDat %>% bind_rows(tmp)
      ...
      tmp <- data.frame(id = dat$id,
                     ...
                     strain.prop = NA,
                    strain.score = NA,
                    species.fitch = dat[,(ncol(dat) - 1)],
                     species.prop = dat[,ncol(dat)],
                     species.score = dat$prop_overlap,
                     genus.fitch = NA,
                     genus.prop = NA,
                    genus.score = NA,
...
for(i in resList$strain){
  ...
  tmp <- data.frame(id = dat$id,
                     strain.fitch = dat$set_val,
                     strain.prop =  dat$prop,
                    strain.score = dat$prop_overlap,
                     species.fitch = NA,
                     species.prop = NA,
                    species.score = NA,
                     genus.fitch = NA,
                    ...
                    order.score = NA,
                     file.name = i,
                    category = "strain")
        tmp <- tmp %>% mutate(keep = ifelse(abs(strain.score) > 0.25, T, ifelse(is.na(strain.score), T,F))) %>% mutate_all(as.character) %>% mutate(score = strain.score)

  resDat <- resDat %>% bind_rows(tmp)
  }else{
      tmp <- data.frame(id = dat$id,
                     strain.fitch = dat[,(ncol(dat) - 1)],
                     strain.prop = dat[,ncol(dat)],
                    strain.score = dat$prop_overlap,
                     species.fitch = NA,
                     species.prop = NA,
                    species.score = NA,
...
for(i in resList$genus){
  ...
  tmp <- data.frame(id = dat$id,
                    ...
                    strain.score = NA,
                     ...
                     species.prop = NA,
                    species.score = NA,
                      genus.fitch = dat$set_val,
                     genus.prop = dat$prop,
                    genus.score = dat$prop_overlap,
                     family.fitch = NA,
                     family.prop = NA,
                    family.score = NA,
                     order.fitch = NA,
                    ...
                    order.score = NA,
                     file.name = i,
                    category = "genus")
        tmp <- tmp %>% mutate(keep = ifelse(abs(genus.score) > 0.25, T, ifelse(is.na(genus.score), T,F))) %>% mutate_all(as.character) %>% mutate(score = genus.score)
  ...
  resDat <- resDat %>% bind_rows(tmp)
      ...
      tmp <- data.frame(id = dat$id,
                    ...
                    strain.score = NA,
                     ...
                     species.prop = NA,
                    species.score = NA,
                     genus.fitch = dat[,(ncol(dat) - 1)],
                     genus.prop = dat[,ncol(dat)],
                    genus.score = dat$prop_overlap,
                     family.fitch = NA,
                     family.prop = NA,
                    family.score = NA,
  ...
  dat <- read.table(paste(filePath, i, "_merged.gff", sep =""), sep = "\t", header = T, fill = T, as.is = T)
      ...
      tmp <- data.frame(id = dat$id,
                    ...
                    species.score = NA,
                     ...
                     genus.prop = NA,
                    genus.score = NA,
                     family.fitch = dat[,(ncol(dat) - 1)],
                     family.prop = dat[,ncol(dat)],
                    family.score = dat$prop_overlap,
                     order.fitch = NA,
                     order.prop = NA,
                    order.score = NA,
                     file.name = i,
                     category = "family")
        tmp <- tmp %>% mutate(keep = ifelse(abs(family.score) > 0.25, T, ifelse(is.na(family.score), T,F))) %>% mutate_all(as.character) %>% mutate(score = family.score)
  ...
  dat <- read.table(paste(filePath, i, "_merged.gff", sep =""), sep = "\t", header = T, fill = T, as.is = T)
      ...
      tmp <- data.frame(id = dat$id,
                    ...
                    genus.score = NA,
                     ...
                     family.prop = NA,
                    family.score = NA,
                     order.fitch = dat[,(ncol(dat) - 1)],
                     order.prop = dat[,ncol(dat)],
                    order.score = dat$prop_overlap,
                     file.name = i,
                     category = "order")
        tmp <- tmp %>% mutate(keep = ifelse(abs(order.score) > 0.25, T, ifelse(is.na(order.score), T,F))) %>% mutate_all(as.character)%>% mutate(score = order.score)
...
ggplot() +
  geom_freqpoly(data = resDatReal, aes(x = as.numeric(score)), colour = "blue")+
  geom_freqpoly(data = resDatRandom, aes(x = as.numeric(score)), colour = "red")+
...
for(i in names(resList)){
  print(i)
  summary <- resDatReal %>% filter(category == i) %>% 
    group_by(strain.fitch, species.fitch, genus.fitch, family.fitch, order.fitch) %>%
    summarise(count = n())
...
sRNAs <- read.table("~/phd/RNASeq/combined_gff_files/version_1/escherichia-shigella-salmonella-enterobacter-klebsiella-serratia_merged.gff", sep = "\t", header = T, fill = T, as.is = T)



sRNAs[sRNAs == "1-0"] <- "0-1"

sRNAs <- sRNAs %>% mutate(escherichia.shigella.salmonella.enterobacter.klebsiella.serratia.fitch = ifelse(escherichia.shigella.salmonella.enterobacter.klebsiella.serratia.fitch == "0-1", 
...
sRNAs <- sRNAs %>% mutate(escherichia.shigella.salmonella.enterobacter.klebsiella.fitch = ifelse(escherichia.shigella.salmonella.enterobacter.klebsiella.fitch == "0-1", 
...
sRNAs <- sRNAs %>% mutate(escherichia.shigella.salmonella.fitch = ifelse(escherichia.shigella.salmonella.fitch == "0-1", 
...
sRNAs <- sRNAs %>% mutate(escherichia.shigella.fitch = ifelse(escherichia.shigella.fitch == "0-1", 
...
sRNAs <- sRNAs %>% mutate(enterobacter.klebsiella.fitch = ifelse(enterobacter.klebsiella.fitch == "0-1", 
...
sRNAsSmall <- sRNAs %>% select(id, escherichia.shigella.salmonella.enterobacter.klebsiella.serratia.fitch, serratia.fitch, escherichia.shigella.salmonella.enterobacter.klebsiella.fitch, enterobacter.klebsiella.fitch, enterobacter, klebsiella, escherichia.fitch, escherichia.shigella.fitch, escherichia.shigella.salmonella.fitch, shigella.fitch, salmonella.fitch)
sRNAsSmall[sRNAsSmall == "1-0"] <- "0"

conservationOrder <- c("shigella.fitch",
                       "escherichia.fitch",
                       "serratia.fitch",
                       "klebsiella",
                       "enterobacter",
...
sRNAsSmall$conserved_to <- ""
for(i in conservationOrder){
  sRNAsSmall$conserved_to[sRNAsSmall[i] == "1"] <- i
}
