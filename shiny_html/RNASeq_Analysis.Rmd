...
```{r rscape, eval = F}
dat <- read.table("~/phd/RNASeq/srna_seqs/version_1/positive_control/positive_control.rscape.cov", comment.char = "#", fill = T)

randomDat <- read.table("~/phd/RNASeq/srna_seqs/version_1/negative_control/negative_control.rscape.out", comment.char = "#", fill = T, sep = "\t")


colnames(dat) <- c("V1", "left_pos", "right_pos", "score", "e.value", "substitutions", "power")
colnames(randomDat) <- c("V1")

dat$score[dat$V1 == "no"] <- 0
dat$e.value[dat$V1 == "no"] <- 10
dat$power[dat$V1 == "no"] <- 0
dat$substitutions[dat$V1 == "no"] <- 0

dat$left_pos[dat$V1 == "no"] <- "-"
dat$right_pos[dat$V1 == "no"] <- "-"
dat$V1[dat$V1 == "no"] <- "-"

randomDat$score <- 0
randomDat$e.value <- 10
randomDat$power <- 0
randomDat$substitutions<- 0



ggplot() +
  geom_freqpoly(data = dat, aes(x = score, y = log(..count..)), binwidth = 5) +
  geom_freqpoly(data = randomDat, aes(x = score, y = log(..count..)), binwidth = 5, colour = "blue")

dat <- dat %>% mutate(response = 1)
randomDat <- randomDat %>% mutate(response = 0)

rocData <- dat %>% bind_rows(randomDat)
...
generaTree <- read.tree("~/phd/RNASeq/alignments/all_alignments/genera_11.guide_tree")
