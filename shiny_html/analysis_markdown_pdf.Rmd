...
-   [*Overview of sRNA*](#section-srna_overview)

-   [*Overview of Methods*](#section-methods)

-   [*Current Figures*](#section-current_figures)

-   [*Summary of strains used*](#section-strains_used)
...
###Current Figures {#current_figures}
...
-   The genome accession for each added genome was looked up on [NCBI](https://www.ncbi.nlm.nih.gov/) using the strain name in the file or by search for the given experiment in the [SRA](https://www.ncbi.nlm.nih.gov/sra/) section.
    ...
    -   Run [*callPeaksforGenome.sh*](#section-callpeaksforgenome.sh) *-g* *\<GCA Accession\>* *-s*
...
-   [*Download data and map reads*](#section-data_download)

-   [*Call peaks on individual RNA-Seq experiments*](#section-call_peaks)

-   [*callPeaksforGenome.sh*](#section-callpeaksforgenome)

    -   [*fetch_genomes_from_GCA.sh*](#section-fetch_genomes_from_gca.sh)

    -   [*sra2plot.1.0.3.sh*](#section-sra2plot.1.0.3.sh)

    -   [*removeProteinCodingRNA.R*](#section-removeproteincodingrna.r)
    
    -   [*run_rnaPeakCalling.R*](#section-run_rnapeakcalling.r)
    
    -   [*sRNA Peak Calling (Current)*](#section-new_rnapeakcalling)

    -   [*Combine GFF files (loop)*](#section-run_combine_gff_over_all_accessions)
    
    -   [*rfamscan*](#section-rfamscan)
    
    -   [*cmscanToGFFWrapper.R*](#section-cmscantogffwrapper.r)

    -   [*cmscanToLookupTable.R*](#section-cmscantolookuptable.r)

    -   [*combine_gff_files.R*](#section-combine_gff_files.r)

-   [*Combining GFF file*](#section-combining_gff_files)

-   [*Get sRNA Sequences*](#section-get_srnas)

    -   [*get_sRNA_sequences.py*](#section-get_sRNA_sequencespy)
    
    -   [*get_sRNA_sequences*](#section-get_sRNA_sequences)
    
    -   [*get_random_srna_sequences_loop*](#section-get_random_srna_sequences_loop)
    
    -   [*get_random_srna_sequence*](#section-get_random_srna_sequence)
    
-   [*Run sRNA nhmmer*](#section-run_srna_nhmmer)

    -   [*single_fasta_files.py*](#section-single_fasta_files.py)
    
    -   [*Make fasta file for each RUF*](#section-single_fasta_files_loop)
    
    -   [*run_srna_nhmmer.sh*](#section-run_srna_nhmmer.sh)
    
    -   [*Run nhmmer over fasta files*](#section-run_srna_nhmmer_loop_initial)
    
    -   [*Run nhmmer over representative genomes*](#section-run_srna_nhmmer_loop_large_alignments)        
...
-   [*Predicting sRNAs*](#section-predicting_srnas)
...
-   These steps are not automated and are newer

```{r new_rnaPeakCalling, eval=F}
...
####run_combine_gff_over_all_accessions
...
####get_sRNA_sequences.py {#get_sRNA_sequencespy}
...
-   Once the new_calls.txt files have been made the predicted sequences are needed.
...
####get_random_srna_sequences_loop
...
-   loop over the single sequences and build models from the predicted sRNAs

    -   done for the known and novel predicted sRNAs
...
-   output is with .fna instead of .stk
...
####run_srna_nhmmer_loop_large_alignments
...
-   from ~/phd/RNASeq/srna_seqs/version_1/seqs_predicted/predicted/alignments
...
##currently on screen 57692 (predicted) and 134352 (known_predicted)
##screen -list to see all
##screen -r 57692 to reattach
```
...
## Known sRNAs and phylogenetic distance {#known_srna}

Go to:

-   [*Top*](#top)

-   [*Run over representative genomes*](#section-subest_of_all_genomes)

-   [*Run csmscan (loop)*](#section-run_csmscan_across_multiple_files)

-   [*Get genome contig pairs*](#section-genome_contig_pairs)

-   [*cmsearch_res_pc*](#section-cmsearch_res_pc)

-   [*cmsearch_res_analysed_genomes*](#section-cmsearch_res_analysed_genomes)

-   [*get_nucelotide_sequences*](#section-get_nucelotide_sequences)

-   [*make_cm_files*](#section-make_cm_files)

-   [*remove_empty_files*](#section-remove_empty_files)

-   [*combine_alignments*](#section-combine_alignments)

-   [*rename_alignment_ids*](#section-rename_alignment_ids)

-   [*reformat_matrix*](#section-reformat_matrix)

-   [*query_target_pairs*](#section-query_target_pairs)

-   [*dna_dists*](#section-dna_dists)
...
###Run over subset of genomes for all genera {#subest_of_all_genomes}

Go to:

-   [*Top*](#top)

-   [*Known sRNAs and distance matrix*](#section-known_srna)

***

-   2 genomes from each genera (from REFSEQ95) were selected as a representative set of genomes
...
-   cmscan run with commands in a file containing the nucleotide files which need to be searched
...
-   output combined with 'cat *.tblout >> ../${group}.tblout'

    -   a second version is now being run across all of the analysed genomes with plot files. This will enable a better analysis of the read depths.

-   reformatted for R with 'cat ${group}.tblout | sed 's/  /\t/g' | tr -s '\t' | sed 's/\t /\t/g' | sed 's/ \t/\t/g' | sed 's/ /\t/3' | sed 's/ /\t/' | sed 's/ /\t/' | sed 's/ /_/g' | sed 's/_!/\t!/g' | tr -s '\t' | sed 's/Pseudomonas\t/Pseudomonas_/g' > ${group}.tab'
...
-  This is needed for lookup steps later as there are genome accessions and contig accessions used by different scripts
...
-   convert the output of the cmsearch for rFam models into the same format that is used for the predicted and negative control datasets

    -   output a file for each rFam model that has the strand, contig and position of sequence
...
-   appears to repeat the above script
...
-   'esl-sfetch --index ${group}.fna' used to index the fasta file

-   once indexed the location files created earlier can be used to extract sequences from the representative genomes that match to each rFam family

    -   a later step (cmalign) uses a lot of memory. To keep it managable the sequences are limited to 20 per file.
    
    -   This is later combined with esl-alimerge
...
-   phylip formatted files have limited name lengths (10 characters I think) so it is necessary to rename IDs and save a lookup table.
...
-   This outputs a file with the query genome, target contig and srna ID
...
-   [*Check negative control for matches to known elements*](#section-check_negaitves)

-   [*compare_groups_for_overlap*](#section-compare_groups_for_overlap)

-   [*remove_not_nc*](#section-remove_not_nc)
...
-   [*redundancy_check*](#section-redundancy_check)

-   [*redundancy_test_example*](#section-redundancy_test_example)

-   [*contigs_and_cordinates_of_matches*](#section-contigs_and_cordinates_of_matches)

-   [*get_stats_on_alignments*](#section-get_stats_on_alignments)

-   [*redundancy_1_R*](#section-redundancy_1_R)

-   [*move_nc_quickly*](#section-move_nc_quickly)

-   [*redundancy_1_in_python*](#section-redundancy_1_in_python)

-   [*redundancy_2*](#section-redundancy_2)

-   [*redundancy_2_overlapping_scores*](#section-redundancy_2_overlapping_scores)

-   [*output_of_list_of_sRNAs_to_combine*](#section-output_of_list_of_sRNAs_to_combine)

-   [*combine_sRNAs_bash*](#section-combine_sRNAs_bash)
...
-   [*Remove redundancy*](#section-remove_redundancy)

***


```{r redundancy_check, eval = F, include=T}
...
-   [*check_high_scoring_negative_control_sequences*](#section-check_high_scoring_negative_control_sequences)

-   [*build_hmm*](#section-build_hmm)

-   [*fetch_all_negative_control_seqs*](#section-fetch_all_negative_control_seqs)

-   [*run_hmmscan*](#section-run_hmmscan)

-   [*hmmscan_summary*](#section-hmmscan_summary)

-   [*hmmscan_analysis*](#section-hmmscan_analysis)

-   [*run_RNAcode.sh*](#section-run_rnacode.sh)

    -   [*rnacode_summary*](#section-rnacode_summary)

    -   [*protein_coding_filter*](#section-protein_coding_filter)

-   [*remove_negative_controls_matching_features*](#section-remove_negative_controls_matching_features)
...
-   [*Check negative controls*](#section-check_negaitve_controls)
...
-   [*run_RNAAlifold.sh*](#section-run_rnaalifold.sh)
    ...
    -   [*rnaalifold_summary*](#section-rnaalifold_summary)
    
    -   [*positive_control_alignments*](#section-positive_control_alignments)
    
    -   [*negative_control_alignments*](#section-negative_control_alignments)

    -   [*rnaalifold_outputs*](#section-rnaalifold_outputs)

-   [*run_rscape.sh*](#section-run_rscape.sh)

    -   [*rscape_summary*](#section-rscape_summary)

-   [*run_alifold.sh*](#section-run_alifold.sh)

    -   [*alifold_summary*](#section-alifold_summary)

-   [*run_rmfamscan.sh*](#section-run_rmfamscan.sh)

-   [*gc_content*](#section-gc_content)

    -   [*get_fasta_sequence*](#section-get_fasta_sequence)
    
    -   [*get_reference_fasta*](#section-get_reference_fasta)
    
    -   [*sRNAGCPercentage.py*](#section-srnagcpercentage.py)

-   [*get RNA labels from Rfam*](#section-get_rna_labels)

    -   [*view_rfam_descriptions*](#section-view_rfam_descriptions)

-   [*sRNA_read_depths*](#section-srna_read_depths)

    -   [*sRNA_read_depths_script*](#section-srna_read_depths_script)
    
    -   [*sRNA_read_depths_predicted*](#section-srna_read_depths_predicted)  

    -   [*combine_predicted_read_depths*](#section-combine_predicted_read_depths)  

    -   [*srna_read_depths_new_calls_pc*](#section-srna_read_depths_new_calls_pc)  

    -   [*match_pc_pred_ids*](#section-match_pc_pred_ids)  

    -   [*summarise_sRNA_read_depths.R*](#section-summarise_srna_read_depths.r)  
...
-   [*Summary*](#section-summary_results)

-   [*conservation_distance*](#section-conservation_distance)

    -   [*conservation_distance_roc_curve*](#section-conservation_distance_roc_curve)
    
    -   [*distance_check*](#section-distance_check)
    
    -   [*predicted_data_upsetr*](#section-predicted_data_upsetr)
    
    -   [*pc_data_upsetr*](#section-pc_data_upsetr)

-   [*read_depths*](#section-read_depths)

    -   [*read_depths_setup*](#section-read_depths_setup)

    -   [*read_depths*](#section-read_depths)

    -   [*read_depths_check*](#section-read_depths_check)

-   [*Covariation*](#section-covariation)

    -   [*rscape_setup*](#section-rscape_setup)

    -   [*rscape_check*](#section-rscape_check)

    -   [*rscape*](#section-rscape)

    -   [*Alifold Covariation*](#section-alifold_cov)

-   [*GC Content*](#section-gc_content_results)

-   [*Secondary Structure*](#section-secondary_structure_results)

    -   [*alifold_setup*](#section-alifold_setup)

    -   [*alifold_check*](#section-alifold_check)
    
    -   [*alifold_results*](#section-alifold_results)
    
    -   [*MFE*](#section-mfe)

-   [*ncRNA motifs*](#section-ncrna_motifs)

-   [*RandomForest*](#section-randomforest)

    -   [*random_forest_data_setup*](#section-random_forest_data_setup)

    -   [*random_forest_setup*](#section-random_forest_setup)
    
    -   [*random_forest*](#section-random_forest)
    
    -   [*correlation_heat_map*](#section-correlation_heat_map)

    -   [*random_forest_roc*](#section-random_forest_roc)

    -   [*random_forsest_predicted_data*](#section-random_forsest_predicted_data)

-   [*Examples*](#section-examples)

    -   [*examples_setup*](#section-examples_setup)
...
```{r predicted_data_upsetr, eval=F}
...
```{r alifold_cov, eval = F, include=T}
...
```{r random_forest_data_setup, eval=T}
load("~/bin/r_git/R/r_files/max_dists_pc.Rda")
load("~/bin/r_git/R/r_files/max_dists_nc.Rda")
