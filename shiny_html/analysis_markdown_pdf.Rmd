...
```{r tree_viewer, echo=T, include=F, eval=F}
  tree <- read.tree("~/phd/RNASeq/genera_tree_predicted.phy")
  p <-ggtree(tree) + 
  # geom_text(aes(label=node), hjust=-.3) +
  # geom_tiplab() + 
  # geom_cladelabel(node="Enterobacterales", label="Enterobacterales", color="red") 
    geom_hilight(node=17, fill="purple")
  p
 


#sub_tree <-  tree_subset(tree, node = 1277, levels_back = 0)
  # sub_tree$node.label
...
ggsave(filename = "~/phd/RNASeq/figures/subset_tree.svg", plot = p, width = 8, height = 16)
...
do 
  ...
  ID=`echo $file | cut -d '.' -f1,2`
  grep ^"#=GS" $file | sort | uniq | cut -d "[" -f1 | cut -d ' ' -f2  | cut -d '|' -f2- |  sed -e "s/$/   $ID/" >> ../../predicted_genomic_sequence_matches.txt
...
for(item in ids){
  ...
  checked_ids <- c(checked_ids, current_ids)
    ...
    write.table(x = current_ids, file = paste("~/phd/RNASeq/srnas/predicted/combined_alignments_ids/", current_ids[1], "_combined_list.txt", sep = ""), append = T, quote = F, row.names = F, col.names = F)
...
save(max_dists_pred, file = "~/bin/r_git/R/r_files/max_dists_pred_v2.Rda")
...
tmp <- max_dists_pred %>% group_by(distance) %>% summarise(count = n()) %>% arrange(-count)
...
p <- ggplot() + 
  geom_freqpoly(data = gc, aes(x = gc.percent, y = ..density.., group = group, color = group), binwidth = 2)
   
p 

ggsave(filename = "~/phd/RNASeq/figures/gc_content.svg", plot = p)
...
predKnownAlifold <-  pcAlifold %>% separate(col = ID, into = c("t1", "t2"), sep = "_") %>% separate(t2, into = "ID", sep = "\\.", extra = "drop") %>% select(-t1) %>% left_join(known_predicted, by = "ID") %>% filter(!is.na(pred)) %>% select(-pred) %>% mutate(group = "Predicted Known") %>% filter(STDV > 0)
...
dists <- pcAlifold %>% bind_rows(predAlifold, ncAlifold, predKnownAlifold) %>% dplyr::rename(max_dist = z_max) 
...
predCumulativeCount <- distsCumulativeCount %>% filter(grepl(pattern = "Predicted", x = group)) %>% arrange(-alifold_z_score) %>%
    mutate(cumulativeCount = cumsum(cumulative_prop))

maxVal <- max(predCumulativeCount$cumulativeCount)

predCumulativeCount <- predCumulativeCount %>% mutate(cumulative_prop = cumulativeCount/maxVal) %>% 
  mutate(group = "Predicted") %>% select(group, alifold_z_score, cumulative_prop)
...
ncMFE <- read.table("~/phd/RNASeq/srna_seqs/version_1/negative_control/large_alignments/negative_control_mfe.txt", sep = "", comment.char = "#", as.is = T, header = F, fill = T)
predMFE <- read.table("~/phd/RNASeq/srna_seqs/version_1/predicted/large_alignments/predicted_mfe.txt", sep = "", comment.char = "#", as.is = T, header = F, fill = T)
...
ncAlifoldScore <- read.table("~/phd/RNASeq/srna_seqs/version_1/negative_control/large_alignments/negative_control_alifold_score.txt",sep = "", fill = T)

predAlifoldScore <- read.table("~/phd/RNASeq/srna_seqs/version_1/predicted/large_alignments/predicted_alifold_score.txt",sep = "", fill = T)
...
dat <- randomForestDat %>% select(-ID) %>% unique %>% select(alifold.score, gc.score, distance, read.max.score, cov.min.eval, motif.max.score, z_max, group)
set.seed(101)
...
load("~/bin/r_git/R/r_files/randomForestDat.Rda")
dat <- randomForestDat %>% select(-ID) %>% unique() %>% mutate(cov.min.eval = -log(cov.min.eval),
...
predDat <- predDat %>% bind_rows(known_predicted)

predDat <- predDat  %>% unique()  %>% select(alifold.score, gc.score, distance, read.max.score, cov.min.eval, motif.max.score, z_max, group, ID)
...
predCounts <- predDat %>% group_by(probability) %>% summarise(count = n()) %>% arrange(probability) %>% ungroup()  %>% mutate(num = as.integer(probability*100)) %>% tidyr::complete(num = seq(from = 0, to = 100, by = 1), fill = list(count = 0)) %>% mutate(probability = num/100) %>% select(-num)




pcCounts <- dat %>% filter(group == "Positive Control")  %>% group_by(probability) %>% summarise(count = n()) %>% arrange(probability) %>% ungroup()  %>% mutate(num = as.integer(probability*100)) %>% tidyr::complete(num = seq(from = 0, to = 100, by = 1), fill = list(count = 0)) %>% mutate(probability = num/100) %>% select(-num)

ncCounts <- dat %>% filter(group == "Negative Control")  %>% group_by(probability) %>% summarise(count = n()) %>% arrange(probability) %>% ungroup()  %>% mutate(num = as.integer(probability*100)) %>% tidyr::complete(num = seq(from = 0, to = 100, by = 1), fill = list(count = 0)) %>% mutate(probability = num/100) %>% select(-num)
...
smoothPred <- zoo::rollapply(TSPred, width = 5, by = 1, FUN = mean, align = "center", partial = T) ##these values might need to be changed
smoothPC <- zoo::rollapply(TSPC, width = 5, by = 1, FUN = mean, align = "center", partial = T) ##these values might need to be changed
smoothNC <- zoo::rollapply(TSNC, width = 5, by = 1, FUN = mean, align = "center", partial = T) ##these values might need to be changed
