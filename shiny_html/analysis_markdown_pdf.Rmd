...
overlaps <- read.csv("~/phd/RNASeq/srna_seqs/version_1/predicted/overlap_values.csv", comment.char = "#", as.is = T, header = T, fill = T)

overlaps <- overlaps %>% mutate(percentage = ifelse(percentage > 1, 100, percentage*100))



ggplot(data = overlaps) +
  geom_freqpoly(aes(x = total), binwidth = 2) +
  geom_freqpoly(aes(x = percentage), colour = "blue", binwidth = 2) +
