...
```{bash run_csmscan_across_multiple_files, eval=F}
for file in *.fna; 
do 
...
do 
  ...
  if [[ $contig_strand == "+" ]]; then 
    ...
    esl-sfetch -c ${contig_start}..${contig_end} ../representative_genomes/representative_genomes.fna $contig        >> RF00177_rep_seqs.fna; else esl-sfetch -r -c ${contig_start}..${contig_end} ../representative_genomes/representative_genomes.fna $contig >> RF00177_rep_seqs.fna;  
...
-   output then aligned with muscle 'muscle -in RF00177_rep_seqs.fna -out RF00177_rep_seqs.mcl'

-   also tried with 'cmalign -o RF00177_rep_seqs.cm RF00177.cm RF00177_rep_seqs.fna '
    
    -   this took too long with to much memory used

-   the .mcl file is converted to phylip format 

    -   esl-reformat phylip RF00177_rep_seqs.mcl > RF00177_rep_seqs.phylip

-   the .phylip file is used with dnadist to create the RF00177_rep.dists file

###reformat_matrix

```{python reformat_matrix, eval=F}
infile=open('/Users/thomasnicholson/phd/RNASeq/RF00177_rep.dists', 'r')
...
save(max_dists, file = "~/bin/r_git/R/r_files/max_dists.Rda")
}
load("~/bin/r_git/R/r_files/max_dists.Rda")
...
dists <- max_dists %>% mutate(group = "Predicted") %>% dplyr::rename(max_dist = distance)
distsCumulativeCount <- cumulativeCounts(dists = dists, smooth = F)

ggplot() +
  geom_line(data = distsCumulativeCount, aes(x= max_dist, y = cumulative_prop))
