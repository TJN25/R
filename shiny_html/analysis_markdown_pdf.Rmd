...
####sRNA_read_depths
```{bash sRNA_read_depths, eval=F, include=F}
##Done in ~/phd/RNASeq/srna_seqs/version_1/${group}/large_alignments/alignments_rnaalifold
group='negative_control'
...
#    fi
    ...
    while read stat_line;
      ...
      if (( $chromosome > 0 ));
        ...
        counter="1"
        for plotfile in ~/phd/RNASeq/genera/${genus}/${genome}.data/plot_files/*_ncRNA.plot;
          do
...
####summarise_sRNA_read_depths.R
```{r summarise_sRNA_read_depths.R, eval=F, include=F}
#!/usr/bin/env Rscript
...
###read_depths
```{r read_depths_setup, eval=F, include=F}
ncRDepth <- read.table("~/phd/RNASeq/srna_seqs/version_1/negative_control/large_alignments/negative_control_read_depths_summary.txt")
...
save(ncRDepth, file = "ncRDepth.Rda")
save(pcRDepth, file = "pcRDepth.Rda")
save(predRDepth, file = "predRDepth.Rda")
...
roc.curve(response = rocDataMaxMax$response, predicted = rocDataMaxMax$max.max.val, 
