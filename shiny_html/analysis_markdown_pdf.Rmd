...
```{r redundancy_check, eval = F}
dat <- read.table("~/phd/RNASeq/srna_seqs/version_1/positive_control/overlapping_models/overlapping_models_ids.txt", sep = "", comment.char = "#", as.is = T, header = F, fill = T)
...
```{r combine_models, eval=F}
oriDat <- read.table("~/phd/RNASeq/srna_seqs/version_1/predicted/original_stats_2.txt", sep = "", comment.char = "#", as.is = T, header = F, fill = T)
oriDat <- oriDat %>% filter(V3 != "")
colnames(oriDat) <- c("ori.descr", "ori.value", "ori.file")

newDat <- read.table("~/phd/RNASeq/srna_seqs/version_1/predicted/new_stats_2.txt", sep = "", comment.char = "#", as.is = T, header = F, fill = T)
...
overlaps_2 <- read.csv("~/phd/RNASeq/srna_seqs/version_1/predicted/overlap_values_3.csv", 
...
overlaps_2 <- overlaps_2 %>% 
  mutate(percentage = ifelse(percentage > 1, percentage*100, percentage*100))
...
sum1 <- overlaps_2 %>% group_by(id1) %>% summarise(id1_count = n())
sum2 <- overlaps_2 %>% group_by(id2) %>% summarise(id2_count = n())

overlaps_unique1 <- overlaps_2 %>% select(id1, percentage, total, count) %>% unique() %>% 
  dplyr::rename(id = id1, percentage1 = percentage, count1 = count, total1 = total)
overlaps_unique2 <- overlaps_2 %>% select(id2, percentage, total, count) %>% unique() %>% 
  dplyr::rename(id = id2, percentage2 = percentage, count2 = count, total2 = total)

overlaps_unique <- overlaps_unique1 %>% full_join(overlaps_unique2)


ggplot(data = overlaps_unique) +
#  geom_freqpoly(aes(x = total), binwidth = 2) +
  geom_freqpoly(aes(x = percentage), colour = "blue", binwidth = 1) +
#  geom_freqpoly(aes(x = count), colour = "red", binwidth = 2) +
  xlim(min = 0, max = 100)

# df <- read.table("~/phd/RNASeq/srna_seqs/version_1/predicted/original_stats_all.txt", sep = "", comment.char = "#", as.is = T, header = F, fill = T)
# 
# df <- df %>% filter(V1 == "Number_of_sequences:")
# 
# 
# ggplot(data = df) +
# #  geom_freqpoly(aes(x = total), binwidth = 2) +
#   geom_freqpoly(aes(x = as.numeric(V2)), colour = "blue", binwidth = 1) +
#   xlim(min = 0, max = 50)
...
```{r combine_models_old, eval = F}
genomicCoorinates <- read.table("~/phd/RNASeq/srna_seqs/version_1/predicted/predicted_genomic_sequence_matches.txt", sep = "", comment.char = "", as.is = T, header = F, fill = T)
head(genomicCoorinates)
colnames(genomicCoorinates) <- c("t1", "detail", "t2", "query.id")

genomicCoorinates <- genomicCoorinates %>% separate(col = detail, into = c("target.contig", "target.coord"), sep = "/", remove = F)

genomicCoorinates <- genomicCoorinates %>% select(query.id, target.contig, target.coord) %>% 
  separate(col = target.coord, into = c("target.start", "target.stop"), sep = "-", remove = F) %>% 
  unique()

targetContigSet <- unique(genomicCoorinates$target.contig)
queryGenomeSet <- unique(genomicCoorinates$query.id)

genomicCoorinates <- genomicCoorinates %>% mutate(target.start = as.numeric(target.start),
                                                  target.stop = as.numeric(target.stop)) %>% 
  arrange(target.start, target.stop)
subDat <- genomicCoorinates %>% filter(target.contig == "NC_004578.1")
overlaps <- c()
...
for(contig in targetContigSet){
  print(contig)
  subDat <- genomicCoorinates %>% filter(target.contig == contig)
...
t1 <- genomicCoorinates %>% filter(query.id == "GCA_000017745.1_1104")
t2 <- genomicCoorinates %>% filter(query.id == "GCA_000017765.1_281")
...
p <-  ggplot()+
  geom_line(data = distsCumulativeCount %>% filter(group != "Predicted"), aes(x = max_dist, y = cumulative_prop, group = group, colour = group))
