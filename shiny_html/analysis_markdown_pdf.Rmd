...
render_toc("~/bin/R/shiny_html/analysis_markdown_pdf.Rmd")
```
...
```{r, echo=F}
load("~/bin/R/r_files/upsetSubsetPC.Rda")
...
```{r, echo=F}
load('~/bin/R/r_files/upsetSubsetPredicted.Rda')
...
```{r, echo=F}
load("~/bin/R/r_files/dists_cum_count.Rda")
...
```{r, echo=F}
load("~/bin/R/r_files/rhoMatrix.Rda")
load("~/bin/R/r_files/sigMatrix.Rda")
...
```{r, echo=F}
load("~/bin/R/r_files/randomForestDistribution.Rda")
...
load("~/bin/R/r_files/accession_info.Rda")
...
load("~/bin/R/r_files/assembly_summary.Rda") # made from ~/phd/RNASeq/SRA_bacteria_RNAseq.txt
...
load("~/bin/R/r_files/sraDatAll.Rda")
...
save(SRA_bacteria_RNASeq_v2, file ="~/bin/R/r_files/SRA_bacteria_RNASeq_v2.Rda")
save(SRA_bacteria_RNASeq_v2_all, file ="~/bin/R/r_files/SRA_bacteria_RNASeq_v2_all.Rda")


load("~/bin/R/r_files/SRA_bacteria_RNASeq_v2.Rda")
load("~/bin/R/r_files/SRA_bacteria_RNASeq_v2_all.Rda")
...
save(sraDatAll, file = "~/bin/R/r_files/sraDatAll.Rda")
}
...
write.table(x = files, file = "~/bin/R/r_files/server_genomes_file_paths.txt", quote = F, row.names = F, col.names = F)
...
```{r tree_viewer, echo=T, include=F, eval=F}
  tree <- read.tree("~/bin/R/r_files/upsetr.tree")

  tbl_tree <- as_tibble(tree)
  
  to_drop <-  c("Azotobacter", "Marinobacter", "Pseudoalteromonas", "Agarivorans", "Vibrio", "Alishewanella",
                "Aggregatibacter", "Mannheimia", "Actinobacillus", "Xenorhabdus", "Providencia", "Proteus",
                "Pantoea", "Brenneria", "Lonsdalea", "Buchnera.", "Wigglesworthia", "Sodalis", "Dickeya",
                "Citrobacter", "Plautiasymbiont", "Shewanella", "Moritella", "Moraxella", "Psychrobacter",
                "Methylomonas", "Cycloclasticus", "Methylococcus", "Francisella", "Pseudoxanthomonas", 
                "Candidatus")
...
ggsave(filename = "~/Documents/Comparative RNA-Seq/upsetr_subset_tree.svg", plot = p, width = 8, height = 16)
...
```{r test_peak_calling_threshold}
plotDat <- read.table("~/phd/RNASeq/genera/Salmonella/GCA_000210855.2.data/plot_files/ERR2020347.plot")
plotDatncRNA <- read.table("~/phd/RNASeq/genera/Escherichia/GCA_000017745.1.data/plot_files/SRR1185095_ncRNA.plot")
callsDatFwd10 <- rnaPeakCalling(dat = plotDatncRNA, col.num = 2, small_peaks = F, plot_threshold = 10)
callsDatFwd11 <- rnaPeakCalling(dat = plotDatncRNA, col.num = 2, small_peaks = F, plot_threshold = 11)
callsDatFwd12 <- rnaPeakCalling(dat = plotDatncRNA, col.num = 2, small_peaks = F, plot_threshold = 12)
callsDatFwd13 <- rnaPeakCalling(dat = plotDatncRNA, col.num = 2, small_peaks = F, plot_threshold = 13)
callsDatFwd14 <- rnaPeakCalling(dat = plotDatncRNA, col.num = 2, small_peaks = F, plot_threshold = 14)
callsDatFwd15 <- rnaPeakCalling(dat = plotDatncRNA, col.num = 2, small_peaks = F, plot_threshold = 15)
callsDatFwd16 <- rnaPeakCalling(dat = plotDatncRNA, col.num = 2, small_peaks = F, plot_threshold = 16)
callsDatFwd17 <- rnaPeakCalling(dat = plotDatncRNA, col.num = 2, small_peaks = F, plot_threshold = 17)
callsDatFwd18 <- rnaPeakCalling(dat = plotDatncRNA, col.num = 2, small_peaks = F, plot_threshold = 18)
callsDatFwd19 <- rnaPeakCalling(dat = plotDatncRNA, col.num = 2, small_peaks = F, plot_threshold = 19)
callsDatFwd20 <- rnaPeakCalling(dat = plotDatncRNA, col.num = 2, small_peaks = F, plot_threshold = 20)

callsDatFwd10 <- callsDatFwd10 %>% mutate(group = "10")
callsDatFwd11 <- callsDatFwd11 %>% mutate(group = "11")
callsDatFwd12 <- callsDatFwd12 %>% mutate(group = "12")
callsDatFwd13 <- callsDatFwd13 %>% mutate(group = "13")
callsDatFwd14 <- callsDatFwd14 %>% mutate(group = "14")
callsDatFwd15 <- callsDatFwd15 %>% mutate(group = "15")
callsDatFwd16 <- callsDatFwd16 %>% mutate(group = "16")
callsDatFwd17 <- callsDatFwd17 %>% mutate(group = "17")
callsDatFwd18 <- callsDatFwd18 %>% mutate(group = "18")
callsDatFwd19 <- callsDatFwd19 %>% mutate(group = "19")
callsDatFwd20 <- callsDatFwd20 %>% mutate(group = "20")

callsDatFwdEscherichia <- callsDatFwd10 %>% bind_rows(callsDatFwd11, callsDatFwd12, callsDatFwd13, callsDatFwd14, callsDatFwd15, callsDatFwd16, callsDatFwd17, callsDatFwd18, callsDatFwd19, callsDatFwd20)

# save(callsDatFwdSalmonela, file = "~/bin/R/r_files/callsDatFwdSalmonela.Rda")

binwidth_val <- 10
max_length <- 200

p <- ggplot() + 
  geom_freqpoly(data = callsDatFwd %>% filter(feature.length < max_length) %>% filter(group %in% c(10,12,14,17,18,20)), aes(x = feature.length, group = group, color = group), binwidth = binwidth_val)
p
ggsave(filename = "~/phd/thesis_figures/SVG/read_depths_threshold_hist_2.svg")

ggplot() + 
  geom_freqpoly(data = callsDatFwd10[callsDatFwd10$feature.length < max_length,], aes(x = feature.length), binwidth = binwidth_val, color = "red") + 
  geom_freqpoly(data = callsDatFwd15[callsDatFwd15$feature.length < max_length,], aes(x = feature.length), binwidth = binwidth_val, color = "blue") + 
  geom_freqpoly(data = callsDatFwd20[callsDatFwd20$feature.length < max_length,], aes(x = feature.length), binwidth = binwidth_val) + 
  geom_freqpoly(data = callsDatFwd18[callsDatFwd18$feature.length < max_length,], aes(x = feature.length), binwidth = binwidth_val, color = "green") + 
  geom_freqpoly(data = callsDatFwd13[callsDatFwd13$feature.length < max_length,], aes(x = feature.length), binwidth = binwidth_val, color = "yellow") + 
  geom_freqpoly(data = callsDatFwd12[callsDatFwd12$feature.length < max_length,], aes(x = feature.length), binwidth = binwidth_val, color = "pink") + 
  geom_freqpoly(data = callsDatFwd14[callsDatFwd14$feature.length < max_length,], aes(x = feature.length), binwidth = binwidth_val, color = "grey") + 
  geom_freqpoly(data = callsDatFwd16[callsDatFwd16$feature.length < max_length,], aes(x = feature.length), binwidth = binwidth_val, color = "brown") + 
  geom_freqpoly(data = callsDatFwd17[callsDatFwd17$feature.length < max_length,], aes(x = feature.length), binwidth = binwidth_val, color = "purple") + 
  geom_freqpoly(data = callsDatFwd19[callsDatFwd19$feature.length < max_length,], aes(x = feature.length), binwidth = binwidth_val, color = "orange") 
...
save(current_ids, file = "~/bin/R/r_files/current_ids.Rda")
...
The matrix that is made is not in an easily usable format:
...
ncRNAcode <- ncDat
save(ncRNAcode, file = "~/bin/R/r_files/ncRNAcode.Rda")
...
predDat <- predDat %>% filter(grepl(pattern = "alignments", hss.id) == F) %>% 
  filter(p.value < 0.05, length > 16)
predRNAcode <- predDat
save(predRNAcode, file = "~/bin/R/r_files/predRNAcode_v2.Rda")
...
save(pcRNAcode, file = "~/bin/R/r_files/pcRNAcode.Rda")
```
...
load(file = "~/bin/R/r_files/current_ids.Rda")
...
save(max_dists_pred_nr, file = "~/bin/R/r_files/max_dists_pred_nr.Rda")
...
run_all <- F
load("~/bin/R/r_files/max_dists_pred.Rda")
# load("~/bin/R/r_files/max_dists_pred_nr.Rda")
load("~/bin/R/r_files/max_dists_pc.Rda")
load("~/bin/R/r_files/max_dists_nc.Rda")
...
```{r large_tree, eval=F, include=F}
  # tree <- read.tree("~/bin/R/r_files/large_labelled_tree.tree")
  tree <- read.tree("~/bin/R/r_files/all_taxa.tree")
  p <-ggtree(tree) + 
  geom_tiplab() +
  xlim(0,0.8)
  p
...
load("~/bin/R/r_files/predDat.Rda")
...
load( file = "~/bin/R/r_files/max_dists_pred.Rda")
load(file = "~/bin/R/r_files/max_dists_pc.Rda")
load(file = "~/bin/R/r_files/max_dists_nc.Rda")
...
save(upsetSubsetPredicted, file='~/bin/R/r_files/upsetSubsetPredicted.Rda')
...
load('~/bin/R/r_files/novel_rufs_gz.Rda')
load('~/bin/R/r_files/known_rufs_gz.Rda')
load('~/bin/R/r_files/noise_rufs_gz.Rda')
...
save(upsetSubsetPC, file= "~/bin/R/r_files/upsetSubsetPC.Rda")
load("~/bin/R/r_files/upsetSubsetPC.Rda")
...
pc_keep <- read.table("~/phd/RNASeq/srna_seqs/version_1/positive_control/large_alignments/positive_control_keep_list.txt")
load("~/bin/R/r_files/randomForestTestDat.Rda")
...
save(upsetSubsetNC, file= "~/bin/R/r_files/upsetSubsetNC.Rda")
load("~/bin/R/r_files/upsetSubsetNC.Rda")
...
ncRDepth <- read.table("~/Downloads/negative_control_read_depths_summary_reads_v1.txt", as.is = T, stringsAsFactors = F)
...
save(ncRDepth, file = "~/bin/R/r_files/ncRDepth.Rda")
save(pcRDepth, file = "~/bin/R/r_files/pcRDepth.Rda")
save(predRDepth, file = "~/bin/R/r_files/predRDepth.Rda")
...
load("~/bin/R/r_files/ncRDepth.Rda")
load("~/bin/R/r_files/pcRDepth.Rda")
load("~/bin/R/r_files/predRDepth.Rda")
...
# 
load("~/bin/R/r_files/ncRNAcode.Rda")
load("~/bin/R/r_files/pcRNAcode.Rda")
load("~/bin/R/r_files/predRNAcode.Rda")
# 
...
save(pcCov, file = "~/bin/R/r_files/pcCovariation.Rda")
save(ncCovRNA, file = "~/bin/R/r_files/ncCovariation.Rda")
save(predCovRNA, file = "~/bin/R/r_files/predCovariation.Rda")
...
load("~/bin/R/r_files/pcCovariation.Rda")
load("~/bin/R/r_files/ncCovariation.Rda")
...
load("~/bin/R/r_files/pcCovariation.Rda")
load("~/bin/R/r_files/ncCovariation.Rda")
load("~/bin/R/r_files/predCovariation.Rda")
...
# save(pcCovAli, file = "~/bin/R/r_files/pcCovAli_v2.Rda")
# save(ncCovAli, file = "~/bin/R/r_files/ncCovAli.Rda")
# save(predCovAli, file = "~/bin/R/r_files/predCovAli.Rda")



load("~/bin/R/r_files/pcCovAli_v2.Rda")
load("~/bin/R/r_files/ncCovAli.Rda")
load("~/bin/R/r_files/predCovAli.Rda")
...
save(pcGC, file= "~/bin/R/r_files/pcGC.Rda")
save(ncGC, file= "~/bin/R/r_files/ncGC.Rda")
save(predGC, file= "~/bin/R/r_files/predGC.Rda")
...
pcGCrf <- pcGC
ncGCrf <- ncGC
save(predGCrf, file = "~/bin/R/r_files/predGCrf.Rda")
save(pcGCrf, file = "~/bin/R/r_files/pcGCrf.Rda")
save(ncGCrf, file = "~/bin/R/r_files/ncGCrf.Rda")
...
load("~/bin/R/r_files/predGCrf.Rda")
load("~/bin/R/r_files/pcGCrf.Rda")
load("~/bin/R/r_files/ncGCrf.Rda")
...
save(genomesGC, file = "~/bin/R/r_files/genomesGC.Rda")
```

Get the genus accessions and the genus names

```{bash genus_accessions, eval = F}
cd ~/phd/RNASeq/genera
do
cd ~/phd/RNASeq/genera/$folder
for file in *.data;
do
accession=`basename $file .data`
echo "$folder $accession" >> ~/phd/RNASeq/genera/genus_accession_lookup.txt
done
cd ~/phd/RNASeq/genera
done
cat genus_accession_lookup.txt > tmp
cat tmp | grep -v "*" > genus_accession_lookup.txt

```


```{r gc_content_rf_relative_setup, eval=F}
load("~/bin/R/r_files/genomesGC.Rda")
...
genomesGC_srna_counts <- genomesGC %>% mutate(srna.per.mb = (number.of.srnas/genome.length)*1000000) %>% filter(!is.na(srna.per.mb)) %>% filter(srna.per.mb < 500)
accessions_lookup <- read.table("~/phd/RNASeq/genera/genus_accession_lookup.txt")

accessions_lookup <- accessions_lookup %>% dplyr::rename(genera = V1, ID = V2)

genomesGC_srna_counts <- genomesGC_srna_counts %>% left_join(accessions_lookup)



save(genomesGC_srna_counts, file = "~/bin/R/r_files/genomesGC_srna_counts.Rda")

load("~/bin/R/r_files/accession_info.Rda")

p <- ggplot() +
  geom_point(data = genomesGC_srna_counts , aes(x = 100 - gc.percent, y = srna.per.mb, group = genera, color = genera))

p

ggsave(filename = "~/phd/thesis_figures/SVG/genome_gc_vs_srna_count.svg", plot = p)
...
pcAlifold<- read.table("~/Downloads/positive_control.alifold", header = F, comment.char = "#", quote = "", sep = "", fill = T, as.is = T, col.names = c( "From",      "To",    "Strand",    "Native.MFE",    "Mean.MFE",     "STDV",      "Z", "ID"))
ncAlifold<- read.table("~/Downloads/negative_control.alifold", header = F, comment.char = "#", quote = "", sep = "", fill = T, as.is = T)

predAlifold<- read.table("~/Downloads/predicted.alifold", header = F, comment.char = "#", quote = "", sep = "", fill = T, as.is = T)
...
save(pcAlifold, file = "~/bin/R/r_files/pcAlifold.Rda")
save(ncAlifold, file = "~/bin/R/r_files/ncAlifold.Rda")
save(predAlifold, file = "~/bin/R/r_files/predAlifold.Rda")
```
...
load("~/bin/R/r_files/pcAlifold.Rda")
load("~/bin/R/r_files/ncAlifold.Rda")
...
load("~/bin/R/r_files/pcAlifold.Rda")
load("~/bin/R/r_files/ncAlifold.Rda")
load("~/bin/R/r_files/predAlifold_v2.Rda")
...
save(pcMFE, file= "~/bin/R/r_files/pcMFE_v2.Rda")
save(ncMFE, file= "~/bin/R/r_files/ncMFE.Rda")
save(predMFE, file= "~/bin/R/r_files/predMFE.Rda")
...
save(pcAlifoldScore, file= "~/bin/R/r_files/pcAlifoldScore_v2.Rda")
save(ncAlifoldScore, file= "~/bin/R/r_files/ncAlifoldScore.Rda")
save(predAlifoldScore, file= "~/bin/R/r_files/predAlifoldScore.Rda")
...
save(pcMotif, file = "~/bin/R/r_files/pcMotif_2.Rda")
save(ncMotif, file = "~/bin/R/r_files/ncMotif_2.Rda")
save(predMotif, file = "~/bin/R/r_files/predMotif_2.Rda")
...
load("~/bin/R/r_files/pcCovariation.Rda") ## pcCov
load("~/bin/R/r_files/ncCovariation.Rda") ## ncCovRNA
...
load("~/bin/R/r_files/pcGC.Rda")
load("~/bin/R/r_files/ncGC.Rda")
...
load("~/bin/R/r_files/pcMFE.Rda")
load("~/bin/R/r_files/ncMFE.Rda")
...
load("~/bin/R/r_files/pcMotif_2.Rda")
load("~/bin/R/r_files/ncMotif_2.Rda")
...
remaining_nc_srnas <- read.table("~/Downloads/negative_control_ids.txt", sep = "\t", fill =  T)
remaining_nc_srnas <- remaining_nc_srnas %>% dplyr::rename(ID = V1) %>% mutate(keep =T)



load("~/bin/R/r_files/pcCovAli.Rda")
load("~/bin/R/r_files/ncCovAli.Rda")
...
load("~/bin/R/r_files/pcAlifoldScore.Rda")
load("~/bin/R/r_files/ncAlifoldScore.Rda")
...
save(dat, file = "~/bin/R/r_files/randomForestDatwithNAs.Rda")
save(datAll, file = "~/bin/R/r_files/randomForestDatwithNAsAll.Rda")
...
# save(randomForestDat, file = "~/bin/R/r_files/randomForestDat_test_2.Rda")


save(randomForestDat, file = "~/bin/R/r_files/randomForestDat.Rda")
save(randomForestDatAll, file = "~/bin/R/r_files/randomForestDatAll.Rda")
```
...
load("~/bin/R/r_files/randomForestDatwithNAs.Rda") ## dat
...
save(randomForestTestDat, file = "~/bin/R/r_files/randomForestTestDat_2.Rda")
...
save(rhoMatrix, file="~/bin/R/r_files/rhoMatrix.Rda")
save(sigMatrix, file="~/bin/R/r_files/sigMatrix.Rda")
...
save(indexes, file = "~/bin/R/r_files/indexes.Rda")

save(training, file = "~/bin/R/r_files/training.Rda")
save(validation1, file = "~/bin/R/r_files/validation1.Rda")

rf_classifier = randomForest(group ~ ., data=training, ntree=100, importance=TRUE)
rf_classifier
save(rf_classifier, file = "~/bin/R/r_files/rf_classifier.Rda")
```
...
training <- dat2[indexes,]
validation1 <- dat2[-indexes,]
save(indexes, file = "~/bin/R/r_files/indexes_gz.Rda")

save(training, file = "~/bin/R/r_files/training_gz.Rda")
save(validation1, file = "~/bin/R/r_files/validation1_gz.Rda")

rf_classifier = randomForest(group ~ ., data=training, ntree=100, importance=TRUE)
rf_classifier
save(rf_classifier, file = "~/bin/R/r_files/rf_classifier_gz.Rda")
```
...
load("~/bin/R/r_files/rf_classifier.Rda")
load("~/bin/R/r_files/training.Rda")
load("~/bin/R/r_files/validation1.Rda")
load("~/bin/R/r_files/randomForestDatAll.Rda")
load("~/bin/R/r_files/randomForestTestDat_2.Rda")
...
load("~/bin/R/r_files/training_gz.Rda")
...
load("~/bin/R/r_files/predCovariation.Rda") ## pcCovRNA
...
load("~/bin/R/r_files/predMFE.Rda")
...
load("~/bin/R/r_files/predMotif_2.Rda")
...
load("~/bin/R/r_files/predAlifoldScore.Rda")
...
save(predDat, file="~/bin/R/r_files/predDat_all.Rda")
predDat_all <- predDat
...
load("~/bin/R/r_files/predRNAcode_v2.Rda")
...
save(predDat, file="~/bin/R/r_files/predDat_gz.Rda")
load("~/bin/R/r_files/predDat_gz.Rda")
...
save(featuresSelected, file="~/bin/R/r_files/featuresSelected_gz.Rda")
...
save(validatation2, file="~/bin/R/r_files/validatation2_gz.Rda")
...
save(randomForestDistribution, file = "~/bin/R/r_files/randomForestDistribution_gz.Rda")

load("~/bin/R/r_files/randomForestDistribution_gz.Rda")

countsCumul <- randomForestDistribution %>% dplyr::rename(max_dist = count) 
...
load("~/bin/R/r_files/featuresSelected_gz.Rda")
featuresSelected <- featuresSelected %>% filter(distance < 0.15)
...
dat <- featuresSelected %>% mutate(cov.min.eval = -log(cov.min.eval)) %>% mutate(cov.min.eval = ifelse(cov.min.eval < 0, 0, cov.min.eval)) %>% mutate(alifold_cov_score = -alifold_cov_score) %>% mutate(alifold_cov_score = ifelse(alifold_cov_score < 0, 0, alifold_cov_score)) %>% mutate(z_max = ifelse(z_max > 0, 0.1, -z_max)) %>% 
  mutate(max_dist = alifold_cov_score + z_max + motif.max.score) %>% filter(p.value > 0.05, group != "Predicted Known")

cumulativeDistribution(dat, show.legend = T, run.ks.test = F) +
    scale_y_continuous(trans = "log10")



dat <- featuresSelected %>% dplyr::rename(max_dist = distance) %>% filter(p.value > 0.05)
...
dat <- featuresSelected %>% select(group, read.max.score) %>% filter(group != "Negative Control") %>% bind_rows(ncRDepth) %>% dplyr::rename(max_dist = read.max.score) %>% filter(max_dist > 0)
...
dat <- featuresSelected %>% mutate(max_dist = ifelse(z_max == 10, 0.1, -z_max))  %>% filter(group != "Predicted", group != "Predicted Known") %>% select(group, max_dist) 
dat <- predDat  %>% mutate(max_dist = ifelse(z_max == 10, 0.1, -z_max))  %>% filter(group != "Predicted Known") %>% select(group, max_dist)  %>% bind_rows(dat)
...
tmp <- featuresSelected %>% mutate(present = ifelse(motif.max.score > 1, T, F)) %>% filter(group != "Predicted Known")

tab <- with(tmp, table(present, group))
...
run_all <- F
if(run_all){
ggsave(filename = "~/phd/RNASeq/figures/all_distributions_log_2.svg", plot = all.p, width = 450, height = 307, units = "mm")
...
load("~/bin/R/r_files/predDat_gz.Rda")
predDat <- predDat %>% left_join(tmp, by = "ID")
...
save(randomForestClassified, file = "~/bin/R/r_files/randomForestClassified.Rda")
...
score_summary(cuttoff = 0.05, column = "distance", dat = featuresSelected %>% filter(p.value > 0.012) %>% filter(distance < 0.15))
score_summary(cuttoff = 100, column = "read.max.score",dat = featuresSelected %>% filter(p.value > 0.012) %>% filter(distance < 0.15))
score_summary(cuttoff = 100, column = "read.max.score", dat = predDat)
score_summary(cuttoff = 10, column = "srna.counts")
score_summary(cuttoff = 3.5, column = "z_max", dat = featuresSelected %>% filter(p.value > 0.012) %>% filter(distance < 0.15) %>%  mutate(z_max  =-z_max))

score_summary(cuttoff = 0.3, column = "probability", dat = randomForestDatAll)
score_summary(cuttoff = 0.3, column = "probability", dat = featuresSelected %>% filter(p.value > 0.05))
score_summary(cuttoff = 0.3, column = "probability", dat = validatation2)
score_summary(cuttoff = 0.3, column = "probability", dat = predDat%>% filter(p.value > 0.05))
score_summary(cuttoff = 0.3, column = "probability")
score_summary(cuttoff = 0.67, column = "probability")

validatation2 %>% filter(probability > 0.3, group == "Positive Control") %>% nrow()


predDat %>% filter(p.value > 0.05, probability < 0.3, group == "Predicted") %>% nrow()


valueSelect <- function(dat, column, group_val, cutoff){
  dat <- dat %>% filter(group == group_val)
  colVal <- match(column, colnames(dat))
  tmpPos <- dat[dat[,colVal] > cutoff,]
  tmpNeg <- dat[dat[,colVal] <= cutoff,]
  tmpPos <- tmpPos %>% filter(!is.na(group))
  tmpNeg <- tmpNeg %>% filter(!is.na(group))
  
  pos_rate <- nrow(tmpPos)/(nrow(tmpPos) + nrow(tmpNeg))
  neg_rate <- 1 -  pos_rate
  
  print(pos_rate)
  print(nrow(tmpPos))
}
valueSelect(dat = featuresSelected %>% filter(distance < 0.15), group = "Negative Control", column = "distance", cutoff = 0.05)

valueSelect(dat = featuresSelected %>% filter(p.value > 0.012) %>% filter(distance < 0.15, read.max.score != 0), group_val = "Positive Control", column = "read.max.score", cutoff = 100)

valueSelect(dat = featuresSelected %>% filter(p.value > 0.012) %>% filter(distance < 0.15) %>%  mutate(z_max  =-z_max), group = "Negative Control", column = "z_max", cutoff = 3.5)


valueSelect(dat = featuresSelected %>% filter(p.value > 0.5) %>% filter(distance < 0.15), group = "Negative Control", column = "probability", cutoff = 0.3)

tmp <- predDat %>% filter(group == "Predicted") %>% mutate(group.2 = ifelse(probability < 0.3, "noise", ifelse(probability > 0.5, "sRNA", "maybe")))

ggplot() +
  geom_point(data = tmp, aes(x = probability, y = read.max.score, group = group.2, color = group.2)) +
  scale_y_continuous(trans = 'log10')

novel_rufs <- predDat %>% filter(group == "Predicted", probability >= 0.3, probability <= 0.5)
known_rufs <- predDat %>% filter(group == "Predicted", probability > 0.5)
noise_rufs <- predDat %>% filter(group == "Predicted", probability < 0.3)

novelCount <- nrow(novel_rufs)
knownCount <- nrow(known_rufs)
noiseCount <- nrow(noise_rufs)
allCount <- predDat %>% filter(group == "Predicted") %>% nrow()
...
save(novel_rufs, file = "~/bin/R/r_files/novel_rufs_gz.Rda")
save(known_rufs, file = "~/bin/R/r_files/known_rufs_gz.Rda")
save(noise_rufs, file = "~/bin/R/r_files/noise_rufs_gz.Rda")

load('~/bin/R/r_files/novel_rufs.Rda')
load('~/bin/R/r_files/known_rufs.Rda')
load('~/bin/R/r_files/noise_rufs.Rda')


ggplot() + 
  geom_freqpoly(data = noise_rufs, aes(x = gc.score, y = ..density..), binwidth = 10, color = 'red') + 
  geom_freqpoly(data = known_rufs, aes(x = gc.score, y = ..density..), binwidth = 10, color = 'blue') +
  geom_freqpoly(data = novel_rufs, aes(x = gc.score, y = ..density..), binwidth = 10, color = 'green')

ggplot() + 
  geom_freqpoly(data = featuresSelected, aes(x = motif.max.score, y = ..density.., group = group, color = group), binwidth = 1)
...
```{r, eval=F}
load("~/bin/R/r_files/distanceMelt.Rda")
...
load(file = "~/bin/R/r_files/featuresSelected_v2.Rda")
...
rocData <- dists %>% filter(group != "Predicted") %>% mutate(response = ifelse(group == "Positive Control", 1, 0)) %>% filter(max_dist < 0.15)
...
max_dists_pc <- max_dists_pc %>% separate(col = id, into = c("ID", "t2"), sep = "\\.")  %>% select(-t2)  %>% left_join(pc_keep, by = "ID") %>% filter(!is.na(keep)) %>% select(-keep) %>% dplyr::rename(srna = ID)

max_dists_nc <-  max_dists_nc %>% separate(col = id, into = c(NA, "srna"), sep = "_", extra = "merge", remove = T) %>% select(-complete)


pcRDepth <- pcRDepth %>% mutate(response = 1) %>% filter(max.val > 0) %>% left_join(max_dists_pc, by = "srna")
ncRDepth <- ncRDepth %>% mutate(response = 0)  %>% left_join(max_dists_nc, by = "srna")
rocData <- pcRDepth %>% bind_rows(ncRDepth) %>% filter(distance < 0.15)
...
max_dists_pc <- max_dists_pc  %>% dplyr::rename(ID = id)
max_dists_nc <-  max_dists_nc %>% dplyr::rename(ID = id)


pcAlifold <- pcAlifold %>% separate(col = ID, into = c(NA, "id"), sep = "_", extra = "merge", remove = T) %>% dplyr::rename(ID = id) %>% mutate(response = 1) %>% filter(STDV > 0) %>% unique() %>% left_join(max_dists_pc, by = "ID")
ncAlifold <- ncAlifold %>% mutate(response = 0) %>% left_join(max_dists_nc, by = "ID")



rocData <- pcAlifold %>% bind_rows(ncAlifold) %>% filter(distance < 0.15)
rocData <- rocData[!is.na(rocData$z_mean),] 
rocData <- rocData[!is.na(rocData$z_max),] 
...
load("~/bin/R/r_files/ncCovariation.Rda")
load("~/bin/R/r_files/predCovariation.Rda")
load("~/bin/R/r_files/featuresSelected.Rda")

load( file = "~/bin/R/r_files/max_dists_pred.Rda")
load(file = "~/bin/R/r_files/max_dists_pc.Rda")
load(file = "~/bin/R/r_files/max_dists_nc.Rda")

max_dists_pc <- max_dists_pc %>% separate(col = id, into = c("ID", "t2"), sep = "\\.") 
max_dists_nc <-  max_dists_nc  %>% separate(col = id, into = c(NA, "ID"), sep = "_", extra = "merge")

 
pcCov <- pcCov %>% separate(col = ID, into = c("ID", NA), sep = "_", extra = "drop")  %>% mutate(response = 1) %>% left_join(max_dists_pc, by = "ID")
ncCovRNA <- ncCovRNA %>% mutate(response = 0) %>% left_join(max_dists_nc, by = "ID")
rocData <- pcCov %>% bind_rows(ncCovRNA) %>% mutate(combined_score = count * mean_score) %>% filter(!is.na(combined_score), distance < 0.15)
roc.curve(response = rocData$response, predicted = rocData$min_eval, 
          main="ROC curve for Covariation Scores")
...
rocData <- pcCovRoc %>% bind_rows(ncCovRoc) %>% filter(!is.na(alifold_cov_score), distance < 0.15) 
roc.curve(response = rocData$response, predicted = rocData$alifold_cov_score, 
          main="ROC curve for Alifold Covariation Scores")
...
pcGC <- pcGCrf %>% left_join(max_dists_pc, by = "ID")%>% mutate(response = 1)
ncGC <- ncGCrf %>% left_join(max_dists_nc, by = "ID")%>% mutate(response = 0)


rocData <- pcGC %>% bind_rows(ncGC) %>% filter(distance < 0.15)
...
pcCovRoc <- featuresSelected %>% filter(group == "Positive Control") %>% mutate(response = 1) 
ncCovRoc <- featuresSelected %>% filter(group == "Negative Control") %>% mutate(response = 0) 
rocData <- pcCovRoc %>% bind_rows(ncCovRoc) %>% filter(!is.na(mfe.score), distance < 0.15) 
...
max_dists_pc <- max_dists_pc %>% separate(col = id, into = c("ID", NA), sep = "\\.") 
max_dists_nc <-  max_dists_nc  %>% separate(col = id, into = c(NA, "ID"), sep = "_", extra = "merge")

ncMotif <- ncMotif %>% mutate(response = 0)%>% separate(col = ID, into = c("t1", "t2", NA, NA), sep = "\\.", remove = T) %>% mutate(id = paste(t1, t2, sep= "."))%>% separate(col = id, into = c(NA, "ID"), sep = "_", extra = "merge", remove = T) %>% select(ID, response, max_score, mean_score, motif_count) %>% left_join(max_dists_nc, by = "ID")
pcMotif <- pcMotif %>% mutate(response = 1) %>% separate(col = ID, into = c("id", NA, NA), sep = "\\.") %>% separate(col = id, into = c(NA, "ID"), sep = "_", extra = "merge") %>% select(ID, response, max_score, mean_score, motif_count) %>% left_join(max_dists_pc, by = "ID")

rocData <- pcMotif %>% bind_rows(ncMotif) %>% filter(distance < 0.15) %>% mutate(motif_sum = motif_count*mean_score)

roc.curve(response = rocData$response, predicted = rocData$motif_sum,
...
load("~/bin/R/r_files/predDat_without_reads.Rda")

predNoReads <- predDat
load("~/bin/R/r_files/predDat_v2.Rda")

cds_ids <- predDat %>% filter(p.value < 0.05) %>% select(ID)

write.table(cds_ids, file = "~/phd/RNASeq/examples/genes/cds_ids.txt", quote = F, col.names = F, row.names = F)
predDat <- predDat %>% select(ID, probability, read.max.score, distance, cov.min.eval, alifold_cov_score, z_max, motif.max.score, srna.counts, mfe.score, p.value)


tmp <- predDat %>% filter(grepl(pattern = "GCA_", ID), cov.min.eval < 1, p.value > 0.05, motif.max.score > 5, z_max < -1, probability > 0.5) %>% select(ID)
```
...
load("~/bin/R/r_files/randomForestDatAll.Rda")
load("~/bin/R/r_files/predDat_all.Rda")
...
```{r srna_pid_figure, eval = F}
pcSRNApid <- read.table("~/phd/RNASeq/srna_seqs/version_1/positive_control/large_alignments/positive_control_snra_pid.txt")

ncSRNApid <- read.table("~/phd/RNASeq/srna_seqs/version_1/negative_control/large_alignments/negative_control_snra_pid.txt")

predSRNApid <- read.table("~/phd/RNASeq/srna_seqs/version_1/predicted/large_alignments/predicted_snra_pid.txt")

colnames(pcSRNApid) <- c("srna.pid", "ID")
colnames(ncSRNApid) <- c("srna.pid", "ID")
colnames(predSRNApid) <- c("srna.pid", "ID")
...
countsCumul <- counts %>% mutate(max_dist = -srna.pid) 
countsCumul <- cumulativeCounts(dists = countsCumul, smooth = F)
...
# 
# p <- ggplot() +
#   geom_freqpoly(data = counts, aes(x = srna.pid, y = ..density.., group = group, color = group), binwidth = 1) +
...
ggsave(filename = "~/phd/thesis_figures/SVG/pid_distribution.svg", plot = p)
}
...
# save(datesDat, file = "~/bin/R/r_files/datesDat.Rda")
...
# save(rnaseq, file = "~/bin/R/r_files/rnaseq.Rda")
...
smooth <- F
  ...
  # save(sequencing_numbers,file = "~/bin/R/r_files/sequencing_numbers.Rda")
  
  load("~/bin/R/r_files/sequencing_numbers.Rda")
...
```{r viral_arcogs, eval=F}
arcogs <- read_csv("~/Downloads/ar14.arCOG.csv", col_names = F)

viral <- read.table("~/phd/PredVirusHost/update_march_2020/arcgos_viral.txt")

viral <- viral %>% dplyr::rename(arcog = V1)
arcogs <- arcogs %>% dplyr::rename(arcog = X7) %>% mutate_all(as.character)

arcogs <- arcogs %>% full_join(viral)


arcogs_viral <- arcogs %>% filter(V2 == "X")


load(file = "~/bin/PredVirusHost//arVOG_arCOGs.Rda")
...
arVOG_arCOG_small <- arVOG_arCOGs %>% dplyr::rename(arVOG = query.name) %>% select(arVOG, arCOG, pfam) %>% unique()

arVOG_pVOGs_small <- arVOG_pVOGs %>% select(arVOG, pVOG) %>% unique()
arVOG_vogDB_small <- arVOG_vogDB %>% select(arVOG, pVOG) %>% unique() %>% dplyr::rename(vogDB = pVOG)

pVOGs_arCOGs <- arVOG_arCOG_small %>% full_join(arVOG_pVOGs_small) %>% 
  full_join(arVOG_vogDB_small)

head(arcogs)
arcogs_small <- arcogs %>% select(arcog, V2, V4) %>% dplyr::rename(category = V2, descr = V4, arCOG = arcog) %>% unique()

pVOGs_arCOGs <- pVOGs_arCOGs %>% left_join(arcogs_small)

```


```{r crispr_archaea, eval=F}
load("~/bin/large_files/cas_arrays_genomes_refseq_83.Rda") 

archaea <- cas_arrays_genomes_refseq_83 %>% filter(domainName == "Archaea")
...
```{r distributio_of_noise, eval=F}
load("~/bin/R/r_files/predDat.Rda")
genus_accessions <- read.table("~/bin/R/r_files/genus_accession_lookup.txt")

predDat <- predDat %>% filter(group == "Predicted") %>% separate(col = ID, into = c("GCA", "accession", NA), remove = F, sep = "_") %>% mutate(V2 = paste(GCA, accession, sep = "_")) %>% left_join(genus_accessions) %>% 
  mutate(category = ifelse(probability <= 0.3, "noise", "srna"))

srna_and_noise <- predDat %>%
  group_by(V1, V2) %>%
  summarize(noise_prop = mean(category == "noise"))



ggplot() +
  geom_bar(data = predDat, aes(x = V1, group = category, fill = category))
```
...
sraDat <- sraDatAll %>% filter(LibraryLayout == "PAIRED")

genusCount <- sraDat %>% group_by(genus) %>% summarise(genus.count =  n())

genomeCount <- sraDat %>% group_by(Organism, genus) %>% summarise(count =  n()) %>% arrange(-count)

genomeCount <- genomeCount %>% full_join(genusCount)

```

```{r, eval=F}
dat_1 <- read.csv("~/Downloads/SraRunInfo_1.csv")

tmp_1 <- dat_1 %>% select(ReleaseDate, Run)  %>% mutate(ReleaseDate = as.Date(ReleaseDate), count = 1, Run = as.character(Run))


dat_2 <- read.csv("~/Downloads/SraRunInfo.csv")
length(unique(dat_2$Experiment))


tmp_2 <- dat_2 %>% select(ReleaseDate, Run)  %>% mutate(ReleaseDate = as.Date(ReleaseDate), count = 1, Run = as.character(Run))


rnaseq <-tmp_1 %>% bind_rows(tmp_2) %>% unique() %>% arrange(ReleaseDate)

ggplot(rnaseq, aes(x=ReleaseDate, y=cumsum(count))) + geom_line()


imgvr <- data.frame(ReleaseDate = c())

mgrast <- data.frame(ReleaseDate = c(2000))
